...
def calc_center(boxes,k):
    ...
    for i in range(k):
        ...
        for j in range(box_number):
            ...
            j_dist = np.sqrt(np.sum((boxes[j] - clusters[i]) ** 2))
        ...
        #------------------------
        near = np.argmin(dist_note)
    return clusters

def kmean_anchors(path='./data/train_xuxixi.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    """在check_anchors中调用
